...
module JekyllOpenSdgPlugins
  class FetchRemoteData < Jekyll::Generator
    ...
    def generate(site)
      ...
      frameworks = { 'sdg' => '' }
      if site.config.has_key?('alternate_frameworks')
        site.config['alternate_frameworks'].each do |framework|
          frameworks[framework['id']] = framework['data_prefix']
        end
      end

      frameworks.each do |framework, framework_prefix|

        is_alt_framework = framework != 'sdg'

        if is_alt_framework
          build_location = framework_prefix
          if !site.data.has_key?(framework)
            site.data[framework] = {}
          end
        else
        ...
        if translated_builds
          ...
          subfolders.each do |language|
            data_target = is_alt_framework ?  site.data[framework][language] : site.data[language]
            ...
            if !data_source.empty?
              if data_target
                ...
                if is_alt_framework
                  site.data[framework][language] = data_source
                else
          ...
          site.config['languages'].each do |language|
            translation_target = is_alt_framework ? site.data[framework]['translations'][language] : site.data['translations'][language]
            translation_source = is_alt_framework ? site.data[framework]['translations'] : site.data[language]['translations']
            if translation_target
              translation_target.deep_merge(translation_source)
            else
              if is_alt_framework
                site.data[framework]['translations'][language] = translation_source
              else
          ...
          if is_alt_framework
            site.data[framework]['reporting'] = site.data[framework][first_language]['reporting']
            site.data[framework]['schema'] = site.data[framework][first_language]['schema']
            site.data[framework]['zip'] = site.data[framework][first_language]['zip']
          else
          ...
          target = is_alt_framework ? site.data[framework] : site.data
